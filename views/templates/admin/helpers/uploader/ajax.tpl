...
<script type="text/javascript">
    ...
    function imageLine(id, image)
    {
        line = $("#lineType").html();
            line = line.replace(/image_id/g, id).replace(/default.jpg/g, 'thumb_'+image);
            $("#imageList").prepend(line);
    }
                        ...
                        for (var i=0; i<data.result.{$name|escape:'html':'UTF-8'}.length; i++) {
                            ...
                            else
                            {
                                ...
                                if (data.result.{$name|escape:'html':'UTF-8'}[i] !== null && data.result.{$name|escape:'html':'UTF-8'}[i].status == 'ok')
                                {
                                    ...
                                    var id = 'imageList_' + response.image_id;
