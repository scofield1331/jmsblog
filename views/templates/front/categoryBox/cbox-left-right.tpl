...
<div id="cat-{$category['category_id']|escape:'html'}" class="category tab-pane fade {if !isset($active)}active in{else}{/if}" >
    ...
    <div class="row">
        ...
        <div class="col-sm-{12/$config['JMSBLOG_CATEGORIES_BOX_COLUMN']}">
            {foreach from=$category['heading'] item=heading}
        ...
        {if $config['JMSBLOG_SHOW_SUB']}
            <div class="col-sm-{12-12/$config['JMSBLOG_CATEGORIES_BOX_COLUMN']*$config['JMSBLOG_SHOW_HEADING']}">
                {foreach from=$category['common'] item=subheading key=k}
                    {if $k mod ($config['JMSBLOG_CATEGORIES_BOX_COLUMN']-$config['JMSBLOG_SHOW_HEADING'])==0}
                    <div class="row">
                    {/if}
                    <div class="col-sm-{12/($config['JMSBLOG_CATEGORIES_BOX_COLUMN']-$config['JMSBLOG_SHOW_HEADING'])}">
                        {include '../postgrid/grid-common.tpl' post=$subheading}
                    </div>
                    {if $k mod ($config['JMSBLOG_CATEGORIES_BOX_COLUMN']-$config['JMSBLOG_SHOW_HEADING'])==$config['JMSBLOG_CATEGORIES_BOX_COLUMN']-$config['JMSBLOG_SHOW_HEADING']-1 || $k==count($category['common'])-1}
                    </div>
                    {/if}
                {/foreach}
            </div>
